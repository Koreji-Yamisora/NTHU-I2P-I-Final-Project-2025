...
class Player:
    ...
    def update(self, x: float, y: float, map: str, direction: str = None, skin: int = None, moving: bool = None) -> None:
...
class PlayerHandler:
    ...
    def update(self, pid: int, x: float, y: float, map_name: str, direction: str = "DOWN", skin: int = 0, moving: bool = False) -> bool:
        with self._lock:
            p = self.players.get(pid)
            if not p:
                return False
            else:
                p.update(float(x), float(y), str(map_name), direction, int(skin), bool(moving))
                return True
