...
class OnlineManager:
    ...
    _event_out_queue: queue.Queue
    _chat_messages: collections.deque
    _last_chat_id: int
    _event_callbacks: list[Callable[[dict], None]]
    ...
    def __init__(self):
        ...
        self._event_out_queue = queue.Queue(maxsize=50)
    ...
    def send_event(self, target_id: int, data: dict) -> bool:
        """Queue a direct event to be sent to another player (e.g. PvP)"""
        if self.player_id == -1:
            return False
        try:
    ...
    def register_event_callback(self, callback: Callable[[dict], None]) -> None:
        """Register a callback for incoming direct events"""
        with self._lock:
    ...
    async def _handle_message(self, message: str) -> None:
        ...
        try:
            ...
            if msg_type == "registered":
                ...
                # Incoming direct event (e.g. from opponent)
                event_data = data.get("data", {})
                # Add sender info if needed: sender_id = data.get("from")
                with self._lock:
