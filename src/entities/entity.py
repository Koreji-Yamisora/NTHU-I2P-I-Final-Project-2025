...
class Entity:
    ...
    def __init__(self, x: float, y: float, game_manager: GameManager) ->None:
        self.animation = Animation('character/ow1.png', ['down', 'left',
            'right', 'up'], 4, (GameSettings.TILE_SIZE, GameSettings.TILE_SIZE)
            )
    ...
    def change_skin(self, skin_idx: int) ->None:
        """Change Skin."""
        x = skin_idx % 6 + 2
        self.animation = Animation(f'character/ow{x}.png', ['down', 'left',
            'right', 'up'], 4, (GameSettings.TILE_SIZE, GameSettings.TILE_SIZE)
            )
    ...
    def to_dict(self) ->dict[str, object]:
        """To Dict."""
        return {'x': self.position.x / GameSettings.TILE_SIZE, 'y': self.
            position.y / GameSettings.TILE_SIZE}

    @classmethod
    def from_dict(cls, data: dict[str, float | int], game_manager: GameManager
        ) ->Entity:
        """From Dict."""
        x = float(data['x'])
        y = float(data['y'])
        return cls(x * GameSettings.TILE_SIZE, y * GameSettings.TILE_SIZE,
            game_manager)
