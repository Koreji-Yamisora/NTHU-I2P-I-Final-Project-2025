...
class Npc(Entity):
    ...
    def _get_los_rect(self) -> pygame.Rect | None:
        ...
        if self.los_direction == Direction.UP:
            los_rect = pygame.Rect(
                npc_rect.centerx - npc_rect.width // 2,
                npc_rect.top - los_length,
                npc_rect.width,
                los_length,
            )
            ...
            los_rect = pygame.Rect(
                npc_rect.left - los_length,
                npc_rect.centery - npc_rect.height // 2,
                los_length,
                npc_rect.height,
            )
    ...
    def from_dict(cls, data: dict, game_manager: GameManager) -> Npc:
        classification = NpcClassification(data.get("classification", "stationary"))
