...
class Shop(Overlay):
            ...
            if idx < 8:
                self.slot_buttons.append(
                    Button(
                        ...
                        self.slots[idx].left,
                        self.slots[idx].top,
                        self.slots[idx].width,
                        self.slots[idx].height,
                ...
                name.rect.left = self.slots[idx].left + crd(self.slots[idx].width).per(
                    5
                )
                name.rect.centery = self.slots[idx].top + crd(self.height).per(50)

                count = Text(f"Available : {item['count'][0]}", 24, "azure")
                count.rect.left = name.rect.right + crd(self.slots[idx].width).per(5)
                count.rect.centery = self.slots[idx].top + crd(self.height).per(50)
    ...
    def action(self, idx):
        ...
        if item["count"][0] > 0:
            item["count"][0] -= 1
            self.item_slot[idx][2].change_text(f"Available : {item['count'][0]}")
