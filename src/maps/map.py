...
class Map:
    ...
    def draw(self, screen: pg.Surface, camera: PositionCamera):
        ...
        if GameSettings.DRAW_HITBOXES:
            for rect in self._collision_map:
                pg.draw.rect(screen, (255, 0, 0), camera.transform_rect(
                    rect), 1)
            for rect in self._bush:
                pg.draw.rect(screen, (0, 255, 0), camera.transform_rect(
                    rect), 1)
    ...
    def check_teleport(self, rect: pg.Rect) ->(Teleport | None):
        ...
        for teleporter in self.teleporters:
            tp_rect = pg.Rect(teleporter.pos.x, teleporter.pos.y,
    ...
    def check_warp(self, rect: pg.Rect) ->(Warp | None):
        ...
        for warp in self.warps:
            warp_rect = pg.Rect(warp.source.x, warp.source.y, GameSettings.
                TILE_SIZE, GameSettings.TILE_SIZE)
            if rect.colliderect(warp_rect):
                return warp
        return None

    def minimap_surface(self, pixelated: int=4, sfact=2, surface: (pg.
        Surface | None)=None) ->pg.Surface:
        """Minimap Surface."""
        if surface is None:
    ...
    def _render_tile_layer(self, target: pg.Surface, layer: pytmx.
        TiledTileLayer, pixelated: int=0, s: int=1) ->None:
        pixel_w = self.tmxdata.width * pixelated
        pixel_h = self.tmxdata.height * pixelated
        ...
        for x, y, gid in layer:
            ...
            if pixelated:
                ...
                image = pg.transform.scale(image, (GameSettings.TILE_SIZE,
                    GameSettings.TILE_SIZE))
                target.blit(image, (x * GameSettings.TILE_SIZE, y *
                    GameSettings.TILE_SIZE))
        if pixelated:
    ...
    def _create_collision_map(self) ->list[pg.Rect]:
        rects = []
        for layer in self.tmxdata.visible_layers:
            if isinstance(layer, pytmx.TiledTileLayer) and ('collision' in
                layer.name.lower() or 'house' in layer.name.lower()):
                for x, y, gid in layer:
                    if gid != 0:
                        """
                        [TODO HACKATHON 4]
                        ...
                        """
                        rects.append(pg.Rect(x * GameSettings.TILE_SIZE, y *
                            GameSettings.TILE_SIZE, GameSettings.TILE_SIZE,
                            GameSettings.TILE_SIZE))
        return rects
    ...
    def from_dict(cls, data: dict) ->'Map':
        ...
        pos = Position(data['player']['x'] * GameSettings.TILE_SIZE, data[
            'player']['y'] * GameSettings.TILE_SIZE)
        warps = [Warp.from_dict(w) for w in data.get('warps', [])]
        return cls(data['path'], tp, pos, warps)

    def to_dict(self):
        """To Dict."""
        return {'path': self.path_name, 'teleport': [t.to_dict() for t in
            self.teleporters], 'warps': [w.to_dict() for w in self.warps],
            'player': {'x': self.spawn.x // GameSettings.TILE_SIZE, 'y': 
            self.spawn.y // GameSettings.TILE_SIZE}}
