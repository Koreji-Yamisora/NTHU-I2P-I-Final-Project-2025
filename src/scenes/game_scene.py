...
class GameScene(Scene):
    ...
    def __init__(self):
        ...
        self.sprite_online = Sprite('ingame_ui/options1.png', (GameSettings
            .TILE_SIZE, GameSettings.TILE_SIZE))
        sw = crd(GameSettings.SCREEN_WIDTH)
        sh = crd(GameSettings.SCREEN_HEIGHT)
        self.menu_button = Button('UI/button_setting.png',
            'UI/button_setting_hover.png', sw.per(3), sh.per(3), 100, 100, 
            lambda : self.setting_overlay.open())
        self.inventory_button = Button('UI/button_backpack.png',
            'UI/button_backpack_hover.png', sw.per(3), sh.per(13), 100, 100,
    ...
    def small_map(self):
        ...
        sw = crd(GameSettings.SCREEN_WIDTH)
        if gh.gm:
            self.minimap_frame = Sprite('UI/raw/UI_Flat_Frame01a.png', (sw //
                4, sw // 4 // gh.gm.current_map.ratio))
    ...
    def update(self, dt: float):
        """Update."""
        if (self.setting_overlay.is_open or self.inventory.is_open or self.
            shop_on):
            pass
        else:
        ...
        if gh.gm:
            ...
            if gh.gm.player:
                ...
                for npc in gh.gm.current_npcs:
                    npc.update(dt)
                self.shop_on = any(npc.shop_ov.is_open for npc in gh.gm.
                    current_npcs)
            ...
            if gh.gm.player and self.online_manager:
                _ = self.online_manager.update(gh.gm.player.position.x, gh.
                    gm.player.position.y, gh.gm.current_map.path_name)
        ...
        if self.map_on and self.mt and input_manager.mouse_pressed(1):
            ...
            if self.tile_pos:
                ...
                if gh.gm and gh.gm.player:
                    start_pos = Position(int(gh.gm.player.position.x //
                        GameSettings.TILE_SIZE), int(gh.gm.player.position.
    ...
    def bfs(self, start: Position, end: Position) ->(list[Position] | None):
        ...
        while queue:
            ...
            if current.x == end.x and current.y == end.y:
                return path
            neighbors = [Position(current.x, current.y - 1), Position(
                current.x, current.y + 1), Position(current.x - 1, current.
                y), Position(current.x + 1, current.y)]
            for next_pos in neighbors:
                if (next_pos.x, next_pos.y) in visited:
                    continue
                if not (0 <= next_pos.x < map_width and 0 <= next_pos.y <
                    map_height):
                    continue
                test_rect = pg.Rect(next_pos.x * GameSettings.TILE_SIZE, 
                    next_pos.y * GameSettings.TILE_SIZE, GameSettings.
                    TILE_SIZE, GameSettings.TILE_SIZE)
                end_rect = pg.Rect(end.x * GameSettings.TILE_SIZE, end.y *
                    GameSettings.TILE_SIZE, GameSettings.TILE_SIZE,
                    GameSettings.TILE_SIZE)
                if gh.gm.current_map.check_teleport(end_rect):
                    if gh.gm.check_collision(test_rect):
                        continue
                elif gh.gm.check_collision(test_rect
    ...
    def get_tile_from_mouse(self) ->(Position | None):
        ...
        if (mouse_x < map_left or mouse_x > map_left + map_width or mouse_y <
            map_top or mouse_y > map_top + map_height):
            return None
            ...
            if self.online_manager and gh.gm.player:
                ...
                for player in list_online:
                    if player['map'] == gh.gm.current_map.path_name:
                        ...
                        pos = cam.transform_position_as_position(Position(
                            player['x'], player['y']))
    ...
    def draw_minimap(self, screen: pg.Surface):
        ...
        if gh.gm:
            if self.mt:
                s = pg.transform.scale(gh.gm.current_map.minimap_surface(4,
                    2), (self.minimap_frame.rect.width - crd(self.
                    minimap_frame.rect.width).per(3), self.minimap_frame.
                    rect.height - crd(self.minimap_frame.rect.width).per(3)))
            else:
                s = pg.transform.scale(gh.gm.current_map.minimap_surface(4,
                    1), (self.minimap_frame.rect.width - crd(self.
                    minimap_frame.rect.width).per(8), self.minimap_frame.
                    rect.height - crd(self.minimap_frame.rect.width).per(8)))
            rect = s.get_rect()
            rect.center = self.minimap_frame.image.get_rect().center
            if gh.gm.player:
                map_w = (gh.gm.current_map.tmxdata.width * GameSettings.
                    TILE_SIZE)
                map_h = (gh.gm.current_map.tmxdata.height * GameSettings.
                    TILE_SIZE)
                ...
                r = pg.Rect(gh.gm.player.position.x * scale_x, gh.gm.player
                    .position.y * scale_y, ts * scale_x, ts * scale_y)
                pg.draw.rect(s, 'RED', r)
                b = pg.Rect((gh.gm.player.position.x - GameSettings.
                    SCREEN_WIDTH // 2) * scale_x, (gh.gm.player.position.y -
                    GameSettings.SCREEN_HEIGHT // 2) * scale_y, 
                    GameSettings.SCREEN_WIDTH * scale_x, GameSettings.
                    SCREEN_HEIGHT * scale_y)
                pg.draw.rect(s, 'AZURE', b, 2)
                if self.tile_pos:
                    w = pg.Rect(self.tile_pos.x * scale_x * GameSettings.
                        TILE_SIZE, self.tile_pos.y * scale_y * GameSettings
                        .TILE_SIZE, ts * scale_x, ts * scale_y)
