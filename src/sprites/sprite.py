...
class ColorSprite:
    ...
    def __init__(self, color: (tuple[int, int, int] | str), size: tuple[int,
        int], alpha: int=255):
...
class Sprite:
    ...
    def draw_hitbox(self, screen: pg.Surface, camera: Optional[
        PositionCamera]=None):
        """Draw hitbox."""
        if camera is not None:
            pg.draw.rect(screen, (255, 0, 0), camera.transform_rect(self.
                rect), 1)
        else:
